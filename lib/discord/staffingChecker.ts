...
export class StaffingCheckerService {
  ...
  async checkTodayStaffing(): Promise<StaffingCheckResult[]> {
    ...
    for (const occurrence of occurrences) {
      ...
      const staffingResult = await this.checkStaffingForEvent(
   ...
   */
  private async checkStaffingForEvent(
    configId: number,
    configName: string,
    eventDate: Date,
    ...
    try {
      // Get myVATSIM events for this date
      ...
      console.error("Error checking staffing:", error);
      throw error;
    }
  }
