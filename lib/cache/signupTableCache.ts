...
export async function getCachedSignupTable(eventId: number, forceRefresh = false): Promise<SignupTableEntry[]> {
  ...
  // 3️⃣ Alle Signups abrufen (inkl. soft-deleted, damit die Tabelle diese durchgestrichen anzeigen kann)
      ...
      try {
        ...
        for (const airport of eventAirportsList) {
          try {
            ...
            // Extract only minimal data (group + restrictions), not full endorsement lists
            airportEndorsements[airport] = extractMinimalEndorsementData(airportResult);
          } catch (err) {
