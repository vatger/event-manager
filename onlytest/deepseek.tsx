...
interface Notification {
  id: number;
  type: 'EVENT_MESSAGE' | 'warning' | 'success' | 'error' | 'event';
  title: string;
  message: string;
  readAt: string | null;
  createdAt: string;
...
export default function NotificationsWidget() {
  ...
  const getNotificationIcon = (type: Notification['type']) => {
    switch (type) {
      ...
      case 'EVENT_MESSAGE':
        return <PartyPopper className="w-4 h-4 text-blue-500" />;
      default:
  ...
  const getEventStatusBadge = (status: Event['status']) => {
    const statusConfig = {
      ...
      COMPLETED: { label: 'Abgeschlossen', variant: 'destructive' as const }
    };
  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      <DropdownMenuContent align="start" className="w-96 max-h-[80vh] overflow-hidden mr-4">
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center p-8 text-center">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                Sie werden benachrichtigt, wenn neue Events oder Updates verfügbar sind.
              </p>
            </div>
          ) : (
                      ...
                      <div className="bg-muted p-2 rounded-md text-xs space-y-1">
                        ...
                        <div className="flex items-center justify-between">
                          <span>
                            ...
                            {notification.event.location && ` • ${notification.event.location}`}
                          </span>
        ...
        <div className="p-1">
          <DropdownMenuItem onClick={load} className="cursor-pointer">
